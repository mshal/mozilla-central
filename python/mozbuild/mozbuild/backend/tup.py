...
class TupCpp(object):
    ...
    def generate_xpidl_rules(self):
        xpidlsrcs = self.tupmk.get_var('XPIDL_SOURCES')
        if xpidlsrcs:
    ...
    def generate_exports(self, exports, namespace=""):
        ...
        for subdir in sorted(children):
            self.generate_exports(children[subdir], namespace=namespace + subdir)

    def generate_pp_rule(self, filename, defines, extra_flags, output_path):
        flags = ' '.join(defines)
        if(extra_flags):
            flags += ' ' + ' '.join(extra_flags)
        output_group = ""
        if filename.endswith('.manifest') and 'webapprt' not in output_path:
            output_group = "| $(MOZ_ROOT)/dist/bin/<installed-manifests>"
        print ": foreach %s | $(MOZ_ROOT)/config/buildid |> ^ Preprocessor %%f -> %%o^ $(PYTHON) $(MOZ_ROOT)/config/Preprocessor.py %s %%f > %%o |> %s/%%b %s" % (filename, flags, output_path, output_group)

    def generate_install_rule(self, filename, output_path):
        output_group = ""
        if filename.endswith('.manifest') and 'webapprt' not in output_path:
            output_group = "| $(MOZ_ROOT)/dist/bin/<installed-manifests>"
        print ": foreach %s |> !cp |> %s/%%b %s" % (filename, output_path, output_group)

    def generate_pp_rules(self):
        no_dist_install = self.tupmk.get_var('NO_DIST_INSTALL')
        if no_dist_install:
            return
        final_target = self.tupmk.get_var_string('FINAL_TARGET')
        extra_pp_components = self.tupmk.get_var('EXTRA_PP_COMPONENTS')
        defines = self.tupmk.get_var('DEFINES')
        defines.extend(self.tupmk.get_var('ACDEFINES'))
        vpath = self.tupmk.get_var('VPATH')

        component_dir = os.path.join(final_target, 'components')
        js_module_dir = self.tupmk.get_var_string('JS_MODULES_PATH')
        if not js_module_dir:
            js_module_dir = os.path.join(final_target, 'modules')

        for component in extra_pp_components:
            self.generate_pp_rule(component, defines, [], component_dir)

        pref_js_exports = self.tupmk.get_var('PREF_JS_EXPORTS')
        if self.tupmk.get_var('GRE_MODULE'):
            pref_dir = 'greprefs'
        elif self.tupmk.get_var('XPI_NAME') or self.tupmk.get_var('LIBXUL_SDK') or self.tupmk.get_var('MOZ_PHOENIX'):
            pref_dir = 'defaults/preferences'
        else:
            pref_dir = 'defaults/pref'
        pref_pp_flags = self.tupmk.get_var('PREF_PP_FLAGS')
        if self.tupmk.get_var('OS_ARCH') == 'WINNT':
            # on win32, pref files need CRLF line endings... see bug 206029
            pref_pp_flags.append('--line-endings=crlf')
        for export in pref_js_exports:
            self.generate_pp_rule(export, defines, pref_pp_flags, os.path.join(final_target, pref_dir))

        extra_components = self.tupmk.get_var('EXTRA_COMPONENTS')
        for component in extra_components:
            self.generate_install_rule(component, component_dir)

        extra_js_modules = self.tupmk.get_var('EXTRA_JS_MODULES')
        for module in extra_js_modules:
            fullpath = self.tupmk.vpath_resolve('.', vpath, module)
            self.generate_install_rule(fullpath, js_module_dir)

        extra_pp_js_modules = self.tupmk.get_var('EXTRA_PP_JS_MODULES')
        for module in extra_pp_js_modules:
            self.generate_pp_rule(module, defines, [], js_module_dir)

        autocfg_js_exports = self.tupmk.get_var('AUTOCFG_JS_EXPORTS')
        autocfg_dest = os.path.join(final_target, 'defaults', 'autoconfig')
        for export in autocfg_js_exports:
            self.generate_install_rule(export, autocfg_dest)

        accessfu_files = self.tupmk.get_var('ACCESSFU_FILES')
        for export in accessfu_files:
            self.generate_install_rule(export, os.path.join(final_target,
                                                            'modules',
                                                            'accessibility'))
    ...
    def generate_chrome_rules(self, extra_manifest_files):
        if os.path.exists("jar.mn"):
            import tup_jar
            final_target = os.path.join(self.tupmk.get_var_string('FINAL_TARGET'),
                                        'chrome')
            jarmaker_flags = self.tupmk.get_var('MAKE_JARS_FLAGS')
            jarmaker_flags.extend(self.tupmk.get_var('XULPPFLAGS'))
            jarmaker_flags.extend(self.tupmk.get_var('DEFINES'))
            jarmaker_flags.extend(self.tupmk.get_var('ACDEFINES'))
            jarmaker_flags.append('jar.mn')
            jarmaker_flags.extend(extra_manifest_files)
            tup_jar = tup_jar.TupJar(final_target, jarmaker_flags,
                                     self.tupmk.moz_root)
            tup_jar.generate_chrome_rules(extra_manifest_files)

class MozBuildMk(object):
    def __init__(self, sandbox):
        self.sandbox = sandbox

    def get_var(self, varname):
        if varname in self.sandbox:
            res = self.sandbox[varname]
            if isinstance(res, list):
                return res
            return [res]
        return []

    def get_var_string(self, varname):
        return ' '.join(self.get_var(varname))
...
if __name__ == '__main__':
    ...
    if sandbox.mozbuild_enabled(os.getcwd(), root_path):
        ...
        if options.target_srcs:
            ...
            if exports:
                tupcpp.generate_exports(exports[0])
...
#    tupcpp.generate_xpidl_rules()
#    tupcpp.generate_pp_rules()
#    tupcpp.generate_chrome_rules(options.extra_manifest_files)
#
#    if options.target_srcs:
#        tupcpp.generate_desc_file()
#        tupcpp.generate_exports()
#    if options.security:
